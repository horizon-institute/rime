...
class ChatSession {
  ...
  getParticipantsString() {
    ...
    for (let participant of this.participants) {
      let name;
      if (participant.name.display != null) name = participant.name.display;
    ...
    if (this.participants.length == 1) {
      ...
      if (this.name) {
        participant_info +=
          "Group chat '" + this.name + "' with " + sessionText.join(', ');
      } else {
...
const searchResult = computed(() => {
  ...
  if (rawEventsSearchResult.value) {
    ...
    result['messageSessions'] =
      rawEventsSearchResult.value.events.messageSessions;

    for (let session of rawEventsSearchResult.value.events.messageSessions) {
      chatSessions.value[session.sessionId] = new ChatSession(
        session.sessionId,
        session.name,
        session.participants,
        session.providerFriendlyName
      );
    }
    ...
    for (let event of rawEventsSearchResult.value.events.events) {
      ...
      if (lastEventForDevice[event.deviceId] !== undefined) {
        newEvent.previousTimestamp =
          lastEventForDevice[event.deviceId].timestamp;
      } else {
...
function* eventsRowGenerator() {
  ...
  let minTime =
    searchResult.value.events.length > 0
      ? searchResult.value.events[0].timestamp
      : null;
